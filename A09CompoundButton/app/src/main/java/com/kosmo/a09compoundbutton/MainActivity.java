...
public class MainActivity extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chk_pol.setChecked(true);   // 경제항목 체크
        // 미리 선택한 항목을 list컬렉션에 저장됨
        checkBoxList.add("정치");
        checkBoxList.add("경제");
        checkBoxList.add("연예");

        // 각 체크박스에 리스너 부착. 선택시 onCheckedChanged()를 호출하게 된다.
        ...
        radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                /*
                - 안드로이드에서 로그를 확인할 때는 Log.로그종류(태그명, 출력값) 형태로 사용한다.
                - 로그의 종류에는 info, debug, error등이 있다.
                - 하단의 Logcat에서 확인할 수 있다.*/
                Log.d("RadioIndex", "라디오 인덱스"+ i);
                // => 라디오 인덱스2131165335 xml에서 선언한 id값은 상수값으로 저장됨.
                RadioButton radio = (RadioButton)findViewById(i);
                // 선택한 라디오버튼의 text값을 얻어와서 문자열로 저장한다.
                radioChecked = radio.getText().toString();
                Toast.makeText(MainActivity.this, radioChecked, Toast.LENGTH_SHORT).show();
            }
        });
        ...
        ////////////////////스피너//////////////////////////////
        /*
         스피너 위젯을 얻어와서 어댑터객체와 연결
            * 어댑터 객체 사용
                - 형식] Adapter<타입매개변수>(컨텍스트, 모양, 항목);
        ...
        /*
         - 앱 실행시 디폴트로 선택된 항목을 지정하려면 반드시 java코드로 처리해야 한다.
         - xml에는 select라는 속성이 별도로 없기 때문이다.
         - setSelection()을 실행하면 onItemSelected() 콜백 메소드가 호출된다.  */
        spinner.setSelection(1);
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                // 선택한 항목의 인덱스가 전달되어 해당 항목을 알 수 있다.
        ...
        button.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view) {
                // 선택한 모든 버튼의 값을 얻어와서 텍스트뷰에 설정함.7
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        /*
        매개변수
            CompundButton : 사용자가 클릭한 버튼객체가 전달됨.
        ...
        if(compoundButton instanceof  CheckBox){
            ...
            else{           // uncheckd상태(체크가 해제된)
                Toast.makeText(this, compoundButton.getText()+"해제함",
                        Toast.LENGTH_SHORT).show();
                // 리스트 컬렉션에서 선택항목 삭제
                checkBoxList.add(compoundButton.getText());
            }
        }
        else if(compoundButton.getId()==R.id.togglebutton){ // 선택한 버튼이 토글버튼일 때...
            if(b==true){
        ...
        else if(compoundButton.getId()==R.id.switchbutton){ // 선택한 버튼이 스위치일 때...
            if(b==true){
